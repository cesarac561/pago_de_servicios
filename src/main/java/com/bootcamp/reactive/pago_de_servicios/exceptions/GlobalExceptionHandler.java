package com.bootcamp.reactive.pago_de_servicios.exceptions;
...
public class GlobalExceptionHandler implements ErrorWebExceptionHandler {
    ...
    public Mono<Void> handle(ServerWebExchange exchange, Throwable ex) {
        ...
        if (ex instanceof ServicioBaseException) {
            var internalException= (ServicioBaseException)ex;
        ...
        if (ex instanceof SuministroBaseException) {
            var internalException= (SuministroBaseException)ex;
            exchange.getResponse().setStatusCode(internalException.getStatus());
            DataBuffer dataBuffer = null;
            try {
                dataBuffer = bufferFactory.wrap(objectMapper.writeValueAsBytes(new HttpError(internalException.getMessage()) ));
